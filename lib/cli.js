...
module.exports = function commandline(argv) {
  ...
  cli
    ...
    .option('-d, --distribute <distribution>', 'only build these distributions', list)
    .option('-s, --storage <storage>', 'specify the storage system', list)
    .option('-w, --watch [port]', 'watch the bundled files for changes', parseInt)
    .option('-u, --update', 'update third party modules in the bundle');
  ...
  /**
   * Update third party modules in the bundle.
   *
   * @param {Boolean} value user wants to update
   * @param {Object} square current square instance
   * @api public
   */
  cli.on('--update', function updater(value, square) {
    var Update = require('./update')
      , update = new Update(square);
  ...
  cli.on('--watch', function watch(port, square) {
    ...
    cli.once('parsed', function parsed() {
      ...
      var watcher = new Watcher(square, port);
    });
  });
  ...
  async.reduce(cli.args, {}, function (memo, bundle, callback) {
    ...
    ['watch', 'update'].forEach(function emit(key) {
