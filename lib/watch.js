...
function Watcher(square, port) {
  ...
  // Emit some events from watcher to pause the spinner momentarily.
  this.square.on('build', function () {
...
Watcher.prototype.watch = function watch(Notify, findit) {
  var self = this
    ...
    , files = _.map(Object.keys(this.square.package.bundle), function (file) {
        return path.resolve(base, file);
      })
    , notifier = new Notify(files)
  ...
  ['css', 'js'].forEach(function forEach(ext) {
    ...
    distributions.push(path.resolve(base, dev), path.resolve(base, min));
  });
