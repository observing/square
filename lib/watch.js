...
Watcher.prototype.watch = function watch(err, modules) {
  var self = this
    , changes = []
    , extras = []
    , distributions = []
    ...
    , files = Object.keys(this.square.package.bundle)
    , notifier = new modules[0](files)
    , finder = modules[1].find(process.env.PWD);

  // prepare distribution possibilities.
  ['css', 'js'].forEach(function forEach(ext) {
    var dist = self.square.package.configuration.dist
      , dev = self.square.template(dist.dev, self.square.tag({ extension: ext, type: 'dev' }))
      , min = self.square.template(dist.min, self.square.tag({ extension: ext, type: 'min' }));
...
/*  Object.keys(processors).forEach(function foreach(extension) {
  ...
  finder.on('file', function found(file) {
    ...
    if (extensions && !~extensions.indexOf(extension)) return;
    ...
    if (~distributions.indexOf(trimmed)) return;
