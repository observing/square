 ...
 * @api public
 */
Watcher.prototype.rebuild = function rebuild() {
  var self = this
    , process = this.config
  ...
  this.square.hold(this.config).parse(process);
};
...
Watcher.prototype.watch = function watch(Notify, findit) {
  ...
  this.adaptable = new Notify(this.config);
  this.adaptable.on('change', _.debounce(this.rebuild.bind(this)), 100);
  this.adaptable.on('error', this.refresher.bind(this));
  ...
  this.square.files(function getFileList(err, list) {
    ...
    self.notifier = new Notify(list);
    self.notifier.on('change', filter);
    self.notifier.on('error', self.refresher.bind(self));
        ...
        finder.on('end', function end() {
          ...
          if (!self.silent) {
            ...
            self.square.logger.notice([
                extras.length
              , 'additional files with extension:'
              , extensions.join(', ')
              , 'were added to square#watcher'
            ].join(' '));
          }

          self.notifier.add(extras);
        });
      });
    }
  });
...
/**
 * Destroy fs.notify of the watcher to prevent any triggers from occuring.
 *
 * @api public
 */
Watcher.prototype.destroy = function destroy() {
  // Drop all listeners of fs.notify
  this.adaptable.removeAllListeners();
