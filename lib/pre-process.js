...
exports.styl = function styl (content, index, total, next) {
  ...
  canihaz.stylus(function canihazStylus (err, stylus) {
    ...
    canihaz.nib(function canihazNib (err, nib) {
      ...
      // Expose warnings from stylus.
      if (stylulz.events) {
        stylulz.events.on('file not found', function urlFileNotFound (msg) {
          logger.warning(msg);
        });
      }
      ...
      if (options.evil) stylulz.define('eval', function evil(str) {
...
// what extensions should we have once compiled
exports.styl.extensions = ['css'];
...
// what extensions should we have once compiled
exports.jade.extensions = ['js', 'html'];
