...
Square.prototype.files = function files(cb) {
  ...
  // Generate an inital batch of files that we need to check it's concated with
  // the dependencies array, as that is not includes in the files or we would
  // get some aweful miss matches when generating outputs.
  var initial = _.unique(
    this.package.meta.tree.concat(this.package.meta.dependencies)
  );
...
Square.prototype.preprocess = function preprocessor(bundle, details, fn) {
  ...
  if (bundle.dependencies && bundle.dependencies.length) {
    content += bundle.dependencies.map(function mapping(key) {
      var prefix = !silent
        ? self.commentWrap('[square] Dependency: ' + key, meta.extension)
        : '';
  ...
  if (!silent) content += this.commentWrap('[square] Bundle: ' + meta.relative, meta.extension);
...
Square.prototype.directive = function directive(data, extension, reference, seen) {
  ...
  function insert(match, commenttype, statement, file) {
    ...
    if (!self.silent) data += self.commentWrap('[square] Directive: ' + location, extension);
