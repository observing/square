...
Square.prototype.files = function files(cb) {
  var bundleMap = this.package.meta.tree.reduce(function bundles(map, bundle) {
    return map[bundle.meta.location] = bundle;
  }, {});
  ...
  async.map(
    ...
    , function compilercheck(file, done) {
        var bundled = bundleMap[file]
          , location = file.meta.location;

        // Check if this file is included in our bundle map, and is so, check if
        // it has a compiler. We might to parse the content and read out the
        // import statements.
        if (!bundled || !bundled.compiler) return done(undefined, [location]);

        // The imports function doesn't include the file we give it, so we have
        // to push it in there later on
        var imported = bundled.compiler.imports(location);
...
Square.prototype.refresh = function refresh(files) {
  ...
  if (!changes.length) return this.emit('build', changes, [ 'css', 'js' ]);
