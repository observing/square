...
Square.prototype.build = function build(platform, extensions, fn) {
  ...
  function reduced(err, result) {
    ...
    async.series([
        function (cb) { self.forEach('once', [], cb); }
//      , function (cb) { async.map(collections, self.forEach.bind(self, 'modifier'), cb); }
//      , function (cb) { async.map(collections, self.forEach.bind(self, 'after'), cb); }
    ], function (err, results) {
      writer(err, _.flatten(results));
    });
  }
