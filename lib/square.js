...
Square.prototype.parse = function parse (data) {
  ...
  /**
   * Generate the weight of a single group or dependency.
   *
   * @param {Number} amount
...
Square.prototype.merge = function merge (extension) {
  ...
  Object.keys(groups).forEach(function eachFilteredGroup(name) {
    ...
    groups[name] = groups[name].filter(function (name) {
