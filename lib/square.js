  ...
  Object.keys(groups).forEach(function eachFilteredGroup(name) {
    ...
    function iterateDependencies (pointer) {
      bundle[pointer].dependencies.forEach(function (current) {
        // recursive dependencies lead to infinite dependencies
        if (pointer === current) {
          return self.logger.critical(
              'Recursive dependency detected on file %s skipping self'
            , pointer
          );
        } else {
    ...
    async.forEachSeries(groups[name], function each (file, next) {
      ...
      // the processing
      count[name][ext + 'seen'] = !(ext + 'seen' in count[name]) ? 0 : ++count[name][ext + 'seen'];
