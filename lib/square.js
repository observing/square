...
/**
 * Output a critical error and die.
 *
 * @param {String}
 * @api private
 */
...
Square.prototype.directive = function directive (data, seen) {
  seen = seen || [];
  var self = this;
  ...
  function insert (match, commenttype, statement, file) {
    ...
    return self.directive(data, seen);
  }
...
Square.prototype.license = function copyright (collection, type) {
  ...
  // object with comment styles per file extension, this ensures that the given
  // license can savely be added to the document
  var commentstyles = {
      js: {
          header: '/*!'
        , body:   ' *'
  ...
  license = license.split(/[\n\r]/g).map(function mapping (line, index, lines) {
    var header = index === 0
      ...
      , style = commentstyles[collection.extension];
