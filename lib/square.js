  ...
  */
  ...
  Object.keys(data.bundle).forEach(function each (file, index, files) {
    ...
    if (!path.existsSync(location)) {
      return this.logger.critical(location + ' does not exist, cant read file');
    }
  ...
  dependencies = dependencies.filter(function (file) {
    if (!data.bundle[file]) {
  ...
  // attach the groups and dependencies for easy lookups
...
Square.prototype.refresh = function refresh (files) {
  ...
  Object.keys(bundle).forEach(function find (file) {
    var meta = bundle[file].meta
      , match = ~files.indexOf(meta.location);
...
Square.prototype.build = function build (extension, fn) {
  ...
  function iterate (err, output) {
    ...
    if (!layer) {
      ...
      return self.write(output.content, 'min', output.extension, done);
    }
  ...
  this.once('merge', function merged (content) {
    ...
    if (!this.stdout) this.write(content, 'dev', extension, done);
  });
...
Square.prototype.write = function write (content, type, extension, fn) {
  content = this.license.apply(this, arguments);

  // how do we need to output the content to stdout or to a file..
  if (this.stdout) return console.log(content);
  ...
  zlib.gzip(content, function compressed (err, data) {
    if (err) return;

    var factor = Buffer.byteLength(content) / data.length;
    this.logger.metric(
        base + ': %s normal, %s compressed with a gzip factor of %s'
      , Buffer.byteLength(content).bytes(1).green
...
Square.prototype.merge = function merge (extension) {
  ...
  Object.keys(groups).forEach(function eachGroup(group) {
    files[group] = self.package.groups[group].filter(function filter (file) {
      var meta = bundle[file].meta
        , ext = meta.extension;
console.log(meta, ext);
      ...
      if (meta.compile) count[ext] = !(ext in count) ? 0 : ++count[ext];
  ...
  async.forEachSeries(files, function each (file, next) {
      ...
      // the processing
      count[ext + 'seen'] = !(ext + 'seen' in count) ? 0 : ++count[ext + 'seen'];
      spec.meta.compile.call(spec, data, count[ext + 'seen'], count[ext], done);
