 ...
 * - cli: CLI mode
 * - silent: don't include comments in output files, false by default
 *
 * @constructor
...
Square.prototype.preprocess = function preprocessor(bundle, details, fn) {
  ...
  //
  config.filename = config.filename || meta.filename;
  config.paths = config.paths || [ meta.path, path.dirname(meta.location) ];
...
Square.prototype.build = function build(platform, extensions, fn) {
  ...
  function done(err, files) {
    ...
    if (err && self.writable) return self.emit('idle', false);
...
Square.prototype.commentWrap = function commentWrap(data, extension, preserve) {
  if (!preserve && this.silent) return '';
