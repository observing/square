 ...
 * @api public
 */
Square.prototype.forEach = function forEach(type, collection, cb) {
  var backup = collection.content
    , self = this
    , processed
    , layers;

  // Fetch the correct middleware layers as plugins are separated by plugin
  // types (if you use the plugin interface correctly).
  layers = this.middleware.filter(function filter(layer) {
    var layertype = layer.interface.prototype.type;

    // Default to modified if no type is set.
    if (!layertype || typeof layertype !== 'string') layertype = 'modified';

    // Check if the current plugin matches the type.
    return layertype === 'plugin::' + type;
  });
...
Square.prototype.build = function build(platform, extensions, fn) {
  ...
  function reduced(err, result) {
    ...
    // First do type:once plugins, then all type:modifier finally type:after.
    async.series([
        self.forEach.bind(self, 'once', []) // don't bother with collections.
      , async.map.bind(async, collections, self.forEach.bind(self, 'modifier'))
      , async.map.bind(async, collections, self.forEach.bind(self, 'after'))
    ], function (err, results) {
      // Drop incomplete results, for instance from type:once plugins.
      writer(err, _.filter(_.flatten(results)));
    });
  }
