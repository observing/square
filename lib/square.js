  ...
  */
  ...
  Object.keys(data.bundle).forEach(function each (file, index, files) {
    ...
    if (!path.existsSync(location)) {
      this.logger.critical(location + ' does not exist, cant read file');
      process.exit(1);
    }
  ...
  // add the group names to the configuration.vars so they can be used in the file names
  this.package.configuration.vars.groups = Object.keys(groups);
...
Square.prototype.refresh = function refresh (files) {
  ...
  Object.keys(bundle).forEach(function find (file) {
    var meta = bundle[file].meta
      , match = files.some(function (file) {
