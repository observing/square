...
Object.defineProperty(Square.prototype, 'distributions', {
  get: function get() {
    ...
    this.middleware.map(function map(layer) {
      distributions.push(layer.prototype.distributions);
    });
 ...
 * @api public
 */
Square.prototype.plugin = function plugin(layer) {
...
Square.prototype.forEach = function forEach(type, collection, cb) {
  ...
  layers = this.middleware.filter(function filter(layer) {
    var layertype = layer.prototype.type;
    ...
    , function iterate(Layer, done) {
        ...
        function yep(data) {
          ...
          self.logger.debug(
              'received an '+(
              err ? 'error' : 'processed'
            )+' response from the '+ plugin.name +' plugin'
          );
        ...
        capture.run(function run() {
          plugin = new Layer(self, collection);
          plugin.every('error', 'data', 'disregard', yep);
        });
      }
