 ...
 * - paths: Array of paths that we shoudl search for plugins
...
Square.prototype.write = function write(collection, fn) {
   ...
   */
  function finished(err) {
    if (err) self.critical('failed to write ' + base, err);

    process.nextTick(function ticktock() {
      self.emit('write', collection, file);
  ...
  if (this.stdout) {
    ...
    fs.writeFile(file, collection.content, finished);
  } else {
    finished();
  }
  ...
  zlib.gzip(collection.content, function compressed(err, data) {
    ...
    // Emit a write.
    self.emit('metrics', collection.content, metrics);
  });
};
...
/**
 * Make square extendable, just like you are used to with Backbone.js
 * constructors.
 */
Square.extend = require('extendable');

/**
 * Small helper function load stuff to the Square's prototypes.
 *
...
Square.cli = function cli(args) {
  return require('./lib/cli')(args || process.argv);
};
...
// Square.load(require('./live'));
//
// Add file watching capabilities.
// Square.load(require('./watch'));
