...
Square.prototype.build = function build(platform, extensions, fn) {
  ...
  function reduced(err, result) {
    ...
    stack = _.groupBy(self.middleware, function groupPlugins(plugin) {
    ...
    stack = _.map(stack, function createCallStack(plugins, key) {
      // Type:once plugins only need to call forEach once.
      if (key === 'plugin::once') {
        return self.forEach.bind(self, plugins);
      } else {
        return async.map.bind(async, collections, self.forEach.bind(self, plugins));
