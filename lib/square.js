...
Square.prototype.preprocess = function preprocess(bundle, details, fn) {
  ...
  meta.compile.call(bundle, content, details.index, details.count, fn);
};
...
Square.prototype.reduce = function reduce(platform, extensions, fn) {
  ...
  count = this.package.files.reduce(function counter(memo, file) {
...
Square.prototype.parse = function parse(data, parseOnly) {
  ...
  var tree = []
    ...
    , extensions = {}
    , dependencies = []
    , size = Object.key(struct.bundle).lenght;
    ...
    if ('dependencies' in bundle) {
      if (Array.isArray(bundle.dependencies)) {
        bundle.dependencies = bundle.dependencies.map(function map(dep) {
          dep = helper.basepath(dep, struct.path);
...
Square.prototype.createMeta = function createMeta(bundle, key) {
  ...
  var directory = this.path
    , location = helper.basepath(key, directory)
  ...
  if (compiler) {
    ...
    if (as && ~compiler.extensions.indexOf(as)) output = as;
    else output = compiler.extensions.pop();
  }
...
Square.prototype.read = function read(args, readOnly) {
  ...
  try {
    ...
    if (args.string) {
      ...
      structure.path = path.dirname(process.env.PWD);  // Folder of the bundle.
      structure.source = args.package;                 // Stringified content.
...
Square.prototype.directive = function directive(data, extension, seen) {
  ...
  function insert(match, commenttype, statement, file) {
    ...
    // Prepend a semicolon to prevent multiple JavaScript function to kill each
    // other if one forgot to append a semi-colon.
    if (extension === 'js' && file.charAt(0) !== ';') {
      file = ';' + file;
    }
 ...
 * @api public
 */
Square.prototype.build = function build(extensions, fn) {
  var platform = ''
    , self = this;
  ...
  function reduced(err, result) {
    ...
    async.map(collections, self.forEach, writer);
  }
  ...
  function writer(err, collections) {
    ...
    async.forEach(collections, self.write, fn);
  }
  ...
  // extensions are optional.. so allow fn only argumenting
  if (typeof extensions === 'function') {
    fn = extensions;
    extensions = null;
  }

  // Make sure that all required variables are filled
  if (!fn) fn = function noop(){};
  if (!extensions || !extensions.length) {
    extensions = Object.keys(this.package.meta.extensions);
  }
