...
Square.prototype.parse = function parse (data) {
  ...
  */
  ...
  // If replaced has similar objects for both distrubtions make sure both are
  // accessable, this check is not fool proof but should suffice.
  if (configuration.replace) {
    literals = configuration.replace;
    if (literals.length !== 2 && !literals.dev && !literals.min) {
      this.package.configuration.replace = {
          min: literals
        , dev: literals
      };
    }
  ...
  this.on('merge', function merged (collection) {
    ...
    function iterate (err, output) {
      ...
      if (!layer) {
        ...
        // Replace the variables in minified as indicated.
        output.content = replacer(
            output
          , 'min'
          , self.package.configuration.replace
        );
    ...
    // Replace the variables in development as indicated.
    collection.content = replacer(
        collection
      , 'dev'
      , this.package.configuration.replace
    );
...
/**
 * Check if the current running version of square is out of date or not.
 *
 * @param {String} branch optional branch to check
 * @api private
 */

Square.prototype.outofdate = function outofdate (branch) {
  var version = this.version || require('../package.json').version
  ...
  canihaz.request(function lazyload (err, request) {
    ...
    request({ uri: url }, function done (err, res, body) {
      ...
      if (latest && latest !== version) {
        self.emit('outofdate', latest, version);
      }
    });
  });
...
Square.prototype.write = function write (collection, type, fn) {
  ...
  // executed
  file = basepath(file, this.package.path);
  this.logger.info('writing', file);

  // write the actual change to disk
  fs.writeFile(file, collection.content, 'utf8', function written (err) {
  ...
  Object.keys(this.package.groups).forEach(function eachGroup(group) {
    ...
    groups[group] = self.package.groups[group].filter(function filter (file) {
      ...
      return ext === extension || (
        meta.compile && ~meta.compile.extensions.indexOf(extension)
      );
    });
  });
