...
Square.prototype.build = function build (extension) {
  ...
  function iterate (err, content) {
    ...
    if (!layer) {
      ...
      return self.write(content, 'min', extension);
    }

    // capture errors that might be caused the middleware layers
    try { layer.call(self, content, extension, iterate); }
...
Square.prototype.merge = function merge (extension) {
  ...
  this.package.dependencies
    .filter(function filter (file) {
      return bundle[file].meta.extension === extension;
    })
    .forEach(function each (file) {
      var spec = bundle[file]
        , content = spec.content.toString('UTF8')
        , surgeon;
  ...
  // emit a merge event
  return this.emit('merge', concat.join('\n'));
};
