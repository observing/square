...
Square.prototype.forEach = function forEach(type, collection, cb) {
  ...
  layers = this.middleware.filter(function filter(layer) {
    ...
    return !layertype                       // No type is always use it
      || typeof layertype !== 'string'      // Or isn't string, then always yes
      || layertype === 'plugin::' + type;   // Or a full Plugin.type match
  });
    ...
    , function iterate(plugin, done) {
        ...
        function yep(data) {
          ...
          self.logger.debug(
              'Received an '+(
              err ? 'error' : 'processed'
            )+' response from the '+ layer.id +' plugin for a '
            + collection.extension + ' file'
          );
...
Square.prototype.build = function build(platform, extensions, fn) {
  ...
  function reduced(err, result) {
    ...
    async.map(collections, self.forEach.bind(self, 'modifier'), writer);
  }
