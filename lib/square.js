...
Square.prototype.bundle = function bundle () {
  ...
  if (args.string) {
    try {
      ...
      this.logger.error('Failed to parse the bundle', e.stack);
...
Square.prototype.merge = function merge (extension) {
  ...
  files = this.package.dependencies.filter(function filter (file) {
    ...
    // maintain a count of each file name
    indexes[ext] = indexes[ext] || 0;
    indexes['total' + ext] = ++indexes[ext];
  ...
  async.forEachSeries(files, function each (file, next) {
      ...
      // we have a pre-process function, so we need to compile the contents of
      // this file first before we can continue, we are adding a counter and
      // total to the compiler as some pre-processors might need to know when
      // it's processing the first or last file
      spec.meta.compile(data, spec, done);
